        ...
        for page_num, page in enumerate(doc):
            ...
            for img_idx, img_info in enumerate(img_list):
                ...
                try:
                    ...
                    if img_width > page_width * 5 and img_height > page_height * 5:
                        continue
        ...
        for block in page_dict.get("blocks", []):
            ...
            for line in block.get("lines", []):
                ...
                for span in line.get("spans", []):
                    ...
                    if text:
                        line_text += text + " "
                        block_font_sizes.append(round(span.get("size", 12), 1))
                        block_is_bold.append(bool(span.get("flags", 0) & 2**4))
                if line_text.strip():
            ...
            if block_font_sizes:
                ...
                is_bold = block_is_bold.count(True) > len(block_is_bold) / 2
            else:
            ...
            is_heading = block_font_size >= heading_threshold or (
                is_bold and block_font_size >= body_font_size + 0.5 and len(block_text) < 150
            )
                ...
                structure.append(
                    {
                        ...
                        "level": _estimate_heading_level(block_font_size, body_font_size),
                        "page": page_num + 1,
                    }
                )
            else:
...
def _estimate_heading_level(font_size: float, body_font_size: float) -> int:
    """
